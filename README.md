...
定时任务由 `Epic Kiosk Claim` 触发，每周三、周五北京时间 08:23 和 20:23 运行。GitHub schedule 使用 UTC，对应 cron 为 `23 0 * * 3,5` 和 `23 12 * * 3,5`。也可以在 `Actions -> Epic Kiosk Claim -> Run workflow` 手动触发领取流程。
