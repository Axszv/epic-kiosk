...
定时任务由 `Epic Kiosk Claim` 触发。当前为临时验证配置：每 5 分钟尝试触发一次，但只有北京时间 2026-06-20 16:40 到 17:30 之间会实际执行领取。验证通过后改回每周三、周五北京时间 08:23 和 20:23。也可以在 `Actions -> Epic Kiosk Claim -> Run workflow` 手动触发领取流程。
