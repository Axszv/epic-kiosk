...
定时任务由 `Epic Kiosk Claim` 触发。当前为临时验证配置：北京时间 13:45 运行，GitHub schedule 使用 UTC，对应 cron 为 `45 5 * * *`。验证通过后改回每周三、周五北京时间 08:23 和 20:23。也可以在 `Actions -> Epic Kiosk Claim -> Run workflow` 手动触发领取流程。
